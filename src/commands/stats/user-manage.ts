...
export async function diableUserStats(routed: RouterRouted) {
  ...
  await routed.message.reply(
    'All stats have now been **Disabled** for your account (across all servers where Kiera is present). If you wish to delete all recorded stats to date (command coming soon!) please reachout via the Kiera Bot Dev Server.'
  )
  return true
}
