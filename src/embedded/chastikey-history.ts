...
export function lockeeHistoryForKeyholder(user: TrackedChastiKeyUser, options: { showRating: boolean }, lockeeStatsCompiled: LockeeStats, apiLockeeLocks: Array<TrackedChastiKeyUserAPIFetchLock>, routerStats: RouterStats, cachedTimestamp: number) {
  var fields: Array<{ name: string; value: string; }> = []
  var pastKeyholders: Array<{ name: string, timeLockedWith: number, numberOfLocks: number, abandonedLocks: number, lastTimeLocked: number }> = []
  const sixMonthAgoTimestamp = ((Date.now() / 1000) - (7890000 * 2))
