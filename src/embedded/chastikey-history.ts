...
export function lockeeHistory(
  ...
  options: { showRating: boolean },
  lockeeStatsCompiled: LockeeStats,
  apiLockeeLocks: Array<TrackedChastiKeyUserAPIFetchLock>,
  ...
  apiLockeeLocks.forEach(lock => {
    ...
    if (lockWasInLast3Months) {
      ...
      if (!isLockAbandoned) {
        stats.last3Months.totalTimeLocked += lock.timestampUnlocked === 0 ? Date.now() / 1000 : lockLength
      }
  ...
  const dateJoinedDaysAgo = lockeeStatsCompiled.joined !== '-' ? `(${Math.round((Date.now() - new Date(lockeeStatsCompiled.joined).getTime()) / 1000 / 60 / 60 / 24)} days ago)` : ''
  var description = ``
...
export function lockeeHistoryPersonal(
  user: TrackedChastiKeyUser,
  options: { showRating: boolean },
  lockeeStatsCompiled: LockeeStats,
  apiLockeeLocks: Array<TrackedChastiKeyUserAPIFetchLock>,
  ...
  apiLockeeLocks.forEach(lock => {
    ...
    if (khIndex > -1) {
      // Abandoned Lock
      if (isLockAbandoned) {
        pastKeyholders[khIndex].abandonedLocks += 1
      }
      // Increment total count
      pastKeyholders[khIndex].numberOfLocks += 1
      // Track time of locked only on unlocked locks
      if (lock.status === 'UnlockedReal') {
        pastKeyholders[khIndex].timeLockedWith += lock.timestampUnlocked - lock.timestampLocked
      }
  ...
  var dateJoinedDaysAgo = lockeeStatsCompiled.joined !== '-' ? `(${Math.round((Date.now() - new Date(lockeeStatsCompiled.joined).getTime()) / 1000 / 60 / 60 / 24)} days ago)` : ''
  ...
  // Only show the ratings if the user has > 5
  if (lockeeStatsCompiled.noOfRatings > 4 && options.showRating) description += ` | Avg Rating \`${lockeeStatsCompiled.averageRating}\` | # Ratings \`${lockeeStatsCompiled.noOfRatings}\``
  description += `\nLongest (completed) \`${Utils.Date.calculateHumanTimeDDHHMM(lockeeStatsCompiled.longestLock)}\` | Average Time Locked (overall) \`${Utils.Date.calculateHumanTimeDDHHMM(
    lockeeStatsCompiled.averageLocked
  )}\``
...
export function lockeeHistoryForKeyholder(
  user: TrackedChastiKeyUser,
  options: { showRating: boolean },
  lockeeStatsCompiled: LockeeStats,
  apiLockeeLocks: Array<TrackedChastiKeyUserAPIFetchLock>,
  ...
  pastKeyholders.forEach((l, i) => {
    if (i > 10) return // Skip, there can only be 20 locks in the db, this means theres an issue server side
    fields.push(pastKHStatsEntryForKH(i, l, fields.length))
  })
  ...
  var dateJoinedDaysAgo = lockeeStatsCompiled.joined !== '-' ? `(${Math.round((Date.now() - new Date(lockeeStatsCompiled.joined).getTime()) / 1000 / 60 / 60 / 24)} days ago)` : ''
  var description = `Locked for \`${lockeeStatsCompiled.monthsLocked}\` months to date`
  description += `\n\`${lockeeStatsCompiled.totalNoOfCompletedLocks}\` locks completed`
  // Only show the ratings if the user has > 5
  if (lockeeStatsCompiled.noOfRatings > 4 && options.showRating) description += `\nAvg Rating \`${lockeeStatsCompiled.averageRating}\` | # Ratings \`${lockeeStatsCompiled.noOfRatings}\``
  description += `\nLongest (completed) \`${Utils.Date.calculateHumanTimeDDHHMM(lockeeStatsCompiled.longestLock)}\``
  description += `\nAverage Time Locked (overall) \`${Utils.Date.calculateHumanTimeDDHHMM(lockeeStatsCompiled.averageLocked)}\``
  description += `\nJoined \`${lockeeStatsCompiled.joined.substr(0, 10)}\` \`${dateJoinedDaysAgo}\``
  description += `\n\n**Past Keyholders (\`<6 months\`):**`

  pastKeyholders.forEach(kh => {
    description += `\n${kh.name}`
  })
  ...
  const messageBlock = {
    embed: {
      title: `${lockeeStatsCompiled.isVerified ? '<:verified:625628727820288000> ' : ''}\`${lockeeStatsCompiled.username}\` - ChastiKey Lockee Statistics - Historical View for Keyholders`,
      description: description,
      color: 9125611,
      timestamp: cachedTimestamp,
      footer: {
...
function pastKHStatsEntryForKH(index: number, kh: { name: string; timeLockedWith: number; numberOfLocks: number; abandonedLocks: number }, numberOfFields: number) {
  // Calculate human readible time for lock from seconds
  const combined = Utils.Date.calculateHumanTimeDDHHMM(kh.timeLockedWith)
  var value = `# locks with: \`${kh.numberOfLocks}\`\n# abandoned with: \`${kh.abandonedLocks}\`\nTotal time*: \`${combined}\``
