...
export async function isCKVerified(routed: RouterRouted) {
  ...
  const kieraUser = new TrackedUser(
    await routed.bot.DB.get<TrackedUser>('users', { $or: [{ id: routed.author.id }, { 'ChastiKey.username': new RegExp(`^${ckUser.username}$`, 'i') }] })
  )

  // Update kiera's record if something is missing
  if ((!ckUser.userID && kieraUser._id) || ckUser.username !== kieraUser.ChastiKey.username) {
    await routed.bot.DB.update('users', { id: routed.author.id }, { $set: { 'ChastiKey.isVerified': ckUser.isVerified, 'ChastiKey.username': ckUser.username } }, { atomic: true })
  }
